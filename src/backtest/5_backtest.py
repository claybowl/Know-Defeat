...
class TSLAShortBacktest:
    """Backtesting system for the TSLA short strategy"""
    ...
    async def get_historical_ticks(self):
        ...
        try:
            async with self.db_pool.acquire() as conn:
                # First, verify data availability
                ...
                self.logger.info(
                    "Data Range Summary:\n"
                    "------------------\n"
                    f"Earliest tick: {time_range['earliest_tick']}\n"
                    f"Latest tick: {time_range['latest_tick']}\n"
                    f"Number of trading days: {time_range['unique_days']}"
                )

                # Fetch the actual tick data
    ...
    async def run_backtest(self):
        ...
        # Process each tick
        for tick in ticks:
            # Process the tick through the bot's strategy
            ...
            if len(self.mock_ib.orders) >= 2:  # Entry and exit make a complete trade
                ...
                trade = {
                    ...
                    'entry_price': tick['price'],  # Use actual prices from tick data
                    'exit_price': tick['price'],
                    'quantity': entry_order['quantity'],
                    'pnl': (entry_order['quantity'] * 
                           (exit_order['price'] - entry_order['price']))
                }
...
async def main():
    ...
    # First, let's find out what data we have available
    ...
    backtest = TSLAShortBacktest(db_pool, start_date, end_date, bot_id=1)
