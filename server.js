...
// Also create an allocation endpoint
app.get('/api/allocation', async (req, res) => {
  try {
    const metrics = await db.getBotMetrics();
    
    // Get top bots by rank score
    const topBots = metrics
      .sort((a, b) => parseFloat(b.rank_score || 0) - parseFloat(a.rank_score || 0))
      .slice(0, 10);
    
    // Get the corresponding bot details
    const bots = await db.getBots();
    const botDetailsMap = bots.reduce((map, bot) => {
      map[bot.bot_id] = bot;
      return map;
    }, {});
    ...
    const allocations = topBots.map(metric => {
      const bot = botDetailsMap[metric.bot_id] || { name: `Bot ${metric.bot_id}`, ticker: 'UNKNOWN', algorithm_type: 'unknown' };
      return {
        bot_id: metric.bot_id,
        name: bot.name,
        ticker: bot.ticker,
        algorithm_type: bot.algorithm_type,
        rank_score: metric.rank_score,
